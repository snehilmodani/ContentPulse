...
const basePayload = {
  ...
  user_id: 'u1',
  trend_run_id: 'run-1',
  domain_profile: { primary_domain: 'technology', region: 'IN-MH' },
};

beforeEach(() => {
  mockPublish.mockClear();
  vi.mocked(XTrendsClient).mockClear();
});

afterEach(() => { vi.clearAllMocks(); });

describe('processTrendHarvesting — happy path', () => {
  it('fetches X trends, inserts trend rows, marks run completed, chains idea-generation', async () => {
    const deps = makeDeps();
    deps.db._queue = [
    ...
    const instance = vi.mocked(XTrendsClient).mock.results[0]!.value as any;
    expect(instance.fetchTrends).toHaveBeenCalledWith('technology', 'IN-MH');
...
describe('processTrendHarvesting — source fetch failure', () => {
  it('logs warn for failed source but still completes run and chains idea-generation', async () => {
    vi.mocked(XTrendsClient).mockImplementationOnce(() => ({
      fetchTrends: vi.fn().mockRejectedValue(new Error('X API 429')),
    }) as any);
