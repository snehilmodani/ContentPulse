...
export const domainProfiles = pgTable(
  'domain_profiles',
  {
    ...
    subDomains: text('sub_domains').array().notNull().default([]),
    targetAudience: text('target_audience'),
    creatorPersona: text('creator_persona'),
    toneOfVoice: text('tone_of_voice').array().notNull().default([]),
    contentMixRatio: jsonb('content_mix_ratio').notNull().default({}),
    region: text('region').notNull().default('IN-MH'),
    inspirationAccounts: text('inspiration_accounts').array().notNull().default([]),
